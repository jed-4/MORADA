import { 
  ...
  type GanttStage, type InsertGanttStage,
  type GanttSubtask, type InsertGanttSubtask
} from "@shared/schema";
...
export interface IStorage {
  ...
  // Gantt Chart Stages CRUD
  getGanttStages(projectId: string): Promise<import("@shared/schema").GanttStage[]>;
  getGanttStage(id: string): Promise<import("@shared/schema").GanttStage | undefined>;
  createGanttStage(stage: import("@shared/schema").InsertGanttStage): Promise<import("@shared/schema").GanttStage>;
  updateGanttStage(id: string, stage: Partial<import("@shared/schema").InsertGanttStage>): Promise<import("@shared/schema").GanttStage | undefined>;
  deleteGanttStage(id: string): Promise<boolean>;
  reorderGanttStages(updates: Array<{id: string, displayOrder: number}>): Promise<void>;
  toggleGanttStageCollapse(id: string): Promise<import("@shared/schema").GanttStage | undefined>;

  // Gantt Chart Subtasks CRUD
  getGanttSubtasks(stageId: string): Promise<import("@shared/schema").GanttSubtask[]>;
  getGanttSubtask(id: string): Promise<import("@shared/schema").GanttSubtask | undefined>;
  createGanttSubtask(subtask: import("@shared/schema").InsertGanttSubtask): Promise<import("@shared/schema").GanttSubtask>;
  updateGanttSubtask(id: string, subtask: Partial<import("@shared/schema").InsertGanttSubtask>): Promise<import("@shared/schema").GanttSubtask | undefined>;
  deleteGanttSubtask(id: string): Promise<boolean>;
  reorderGanttSubtasks(updates: Array<{id: string, displayOrder: number}>): Promise<void>;

  // Gantt Chart Auto-dependency Helpers
  recalculateStageProgress(stageId: string): Promise<import("@shared/schema").GanttStage | undefined>;
...
export class DbStorage implements IStorage {
  ...
  // Gantt Chart Stages CRUD
  async getGanttStages(projectId: string): Promise<import("@shared/schema").GanttStage[]> {
    try {
      const stages = await db.select()
        .from(schema.ganttStages)
        .where(eq(schema.ganttStages.projectId, projectId))
        .orderBy(asc(schema.ganttStages.displayOrder));
      return stages;
    } catch (error) {
      console.error("Database error in getGanttStages:", error);
      return [];
    }
  }

  async getGanttStage(id: string): Promise<import("@shared/schema").GanttStage | undefined> {
    try {
      const [stage] = await db.select()
        .from(schema.ganttStages)
        .where(eq(schema.ganttStages.id, id))
        .limit(1);
      return stage;
    } catch (error) {
      console.error("Database error in getGanttStage:", error);
      return undefined;
    }
  }

  async createGanttStage(stageData: import("@shared/schema").InsertGanttStage): Promise<import("@shared/schema").GanttStage> {
    try {
      const [stage] = await db.insert(schema.ganttStages)
        .values(stageData)
        .returning();
      return stage;
    } catch (error) {
      console.error("Database error in createGanttStage:", error);
      throw error;
    }
  }

  async updateGanttStage(id: string, stageData: Partial<import("@shared/schema").InsertGanttStage>): Promise<import("@shared/schema").GanttStage | undefined> {
    try {
      const [updated] = await db.update(schema.ganttStages)
        .set({ ...stageData, updatedAt: new Date() })
        .where(eq(schema.ganttStages.id, id))
        .returning();
      return updated;
    } catch (error) {
      console.error("Database error in updateGanttStage:", error);
      return undefined;
    }
  }

  async deleteGanttStage(id: string): Promise<boolean> {
    try {
      await db.delete(schema.ganttStages)
        .where(eq(schema.ganttStages.id, id));
      return true;
    } catch (error) {
      console.error("Database error in deleteGanttStage:", error);
      return false;
    }
  }

  async reorderGanttStages(updates: Array<{id: string, displayOrder: number}>): Promise<void> {
    try {
      await db.transaction(async (tx) => {
        for (const update of updates) {
          await tx.update(schema.ganttStages)
            .set({ displayOrder: update.displayOrder, updatedAt: new Date() })
            .where(eq(schema.ganttStages.id, update.id));
        }
      });
    } catch (error) {
      console.error("Database error in reorderGanttStages:", error);
      throw error;
    }
  }

  async toggleGanttStageCollapse(id: string): Promise<import("@shared/schema").GanttStage | undefined> {
    try {
      const stage = await this.getGanttStage(id);
      if (!stage) return undefined;

      const [updated] = await db.update(schema.ganttStages)
        .set({ isCollapsed: !stage.isCollapsed, updatedAt: new Date() })
        .where(eq(schema.ganttStages.id, id))
        .returning();
      return updated;
    } catch (error) {
      console.error("Database error in toggleGanttStageCollapse:", error);
      return undefined;
    }
  }

  // Gantt Chart Subtasks CRUD
  async getGanttSubtasks(stageId: string): Promise<import("@shared/schema").GanttSubtask[]> {
    try {
      const subtasks = await db.select()
        .from(schema.ganttSubtasks)
        .where(eq(schema.ganttSubtasks.stageId, stageId))
        .orderBy(asc(schema.ganttSubtasks.displayOrder));
      return subtasks;
    } catch (error) {
      console.error("Database error in getGanttSubtasks:", error);
      return [];
    }
  }

  async getGanttSubtask(id: string): Promise<import("@shared/schema").GanttSubtask | undefined> {
    try {
      const [subtask] = await db.select()
        .from(schema.ganttSubtasks)
        .where(eq(schema.ganttSubtasks.id, id))
        .limit(1);
      return subtask;
    } catch (error) {
      console.error("Database error in getGanttSubtask:", error);
      return undefined;
    }
  }

  async createGanttSubtask(subtaskData: import("@shared/schema").InsertGanttSubtask): Promise<import("@shared/schema").GanttSubtask> {
    try {
      const [subtask] = await db.insert(schema.ganttSubtasks)
        .values(subtaskData)
        .returning();
      
      // Recalculate parent stage progress
      await this.recalculateStageProgress(subtask.stageId);
      
      return subtask;
    } catch (error) {
      console.error("Database error in createGanttSubtask:", error);
      throw error;
    }
  }

  async updateGanttSubtask(id: string, subtaskData: Partial<import("@shared/schema").InsertGanttSubtask>): Promise<import("@shared/schema").GanttSubtask | undefined> {
    try {
      const [updated] = await db.update(schema.ganttSubtasks)
        .set({ ...subtaskData, updatedAt: new Date() })
        .where(eq(schema.ganttSubtasks.id, id))
        .returning();
      
      if (updated) {
        // Recalculate parent stage progress
        await this.recalculateStageProgress(updated.stageId);
      }
      
      return updated;
    } catch (error) {
      console.error("Database error in updateGanttSubtask:", error);
      return undefined;
    }
  }

  async deleteGanttSubtask(id: string): Promise<boolean> {
    try {
      const subtask = await this.getGanttSubtask(id);
      if (!subtask) return false;

      await db.delete(schema.ganttSubtasks)
        .where(eq(schema.ganttSubtasks.id, id));
      ...
      console.error("Database error in deleteGanttSubtask:", error);
      return false;
    }
  }

  async reorderGanttSubtasks(updates: Array<{id: string, displayOrder: number}>): Promise<void> {
    try {
      await db.transaction(async (tx) => {
        for (const update of updates) {
          await tx.update(schema.ganttSubtasks)
            .set({ displayOrder: update.displayOrder, updatedAt: new Date() })
            .where(eq(schema.ganttSubtasks.id, update.id));
        }
      });
    } catch (error) {
      console.error("Database error in reorderGanttSubtasks:", error);
      throw error;
    }
  }

  // Gantt Chart Auto-dependency Helpers
  async recalculateStageProgress(stageId: string): Promise<import("@shared/schema").GanttStage | undefined> {
    try {
      const subtasks = await this.getGanttSubtasks(stageId);
      
      if (subtasks.length === 0) {
        return await this.getGanttStage(stageId);
      }

      // Calculate average progress
      const totalProgress = subtasks.reduce((sum, subtask) => sum + subtask.progressPercent, 0);
      ...
      // Calculate date range from subtasks
      const startDates = subtasks.map(st => new Date(st.startDate));
      const endDates = subtasks.map(st => new Date(st.endDate));
      ...
      const [updated] = await db.update(schema.ganttStages)
        ...
        .where(eq(schema.ganttStages.id, stageId))
        .returning();

      return updated;
    } catch (error) {
      console.error("Database error in recalculateStageProgress:", error);
      return undefined;
    }
  }
  ...
  async calculateCriticalPath(projectId: string): Promise<void> {
    try {
      ...
      const stages = await this.getGanttStages(projectId);
      
      // Simple critical path: stages with longest duration on the critical path
      const stagesWithDuration = stages.map(stage => ({
        ...stage,
        duration: new Date(stage.endDate).getTime() - new Date(stage.startDate).getTime()
      }));

      // Sort by duration descending
      const sortedStages = stagesWithDuration.sort((a, b) => b.duration - a.duration);

      // Assign critical path rank to top stages (simplified)
      await db.transaction(async (tx) => {
        for (let i = 0; i < sortedStages.length; i++) {
          const stage = sortedStages[i];
          const rank = i < Math.ceil(sortedStages.length / 3) ? i + 1 : null;
          
          await tx.update(schema.ganttStages)
            .set({ criticalPathRank: rank, updatedAt: new Date() })
            .where(eq(schema.ganttStages.id, stage.id));
        }
      });
    } catch (error) {
      console.error("Database error in calculateCriticalPath:", error);
      throw error;
    }
  }
