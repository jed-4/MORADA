...
export class DbStorage implements IStorage {
  ...
  async generateRecurringTasks(companyId: string): Promise<{ generated: number }> {
    try {
      ...
      // Build Set of existing task keys (templateId:date)
      const existingTaskKeys = new Set<string>();
      for (const task of existingTasks) {
        const taskData = task as any;
        if (taskData.templateId && taskData.dueDate) {
          const key = getRecurringTaskKey(taskData.templateId, taskData.dueDate);
      ...
      for (const template of templates) {
        ...
        for (const instance of instances) {
          ...
          // Add to existing keys to avoid duplicates within this generation batch
          const key = getRecurringTaskKey(instance.templateId, instance.dueDate);
          existingTaskKeys.add(key);
