...
export interface EstimatesViewProps {
  /** Scope to one project. Omitted = the cross-project "All Projects" view. */
  projectId?: string;
  /** Rendered inside the project shell, which supplies its own chrome. */
  embedded?: boolean;
}
...
export default function EstimatesView({ projectId, embedded }: EstimatesViewProps = {}) {
  ...
  const handleNewEstimate = () => {
    setLocation(scoped ? `/projects/${projectId}/estimates/new` : '/estimates/new');
  };


  // Fetch estimates — every project, or just this one.
  const { data: estimates = [], isLoading: estimatesLoading } = useQuery<Estimate[]>({
    queryKey: projectId ? ["/api/estimates", projectId] : ["/api/estimates"],
    queryFn: async () => {
      const response = await fetch(
        projectId ? `/api/estimates?projectId=${projectId}` : `/api/estimates`,
        { credentials: "include" },
      );
      if (!response.ok) {
  ...
  const filteredEstimates = useMemo(() => {
    return estimates.filter(estimate => {
      ...
      const matchesProject = scoped || selectedProject === 'All' || estimate.projectId === selectedProject;
  ...
  }, [estimates, searchTerm, selectedProject, selectedStatus, projects, showArchived, scoped]);
  ...
  // Revisions of one estimate are one thing, not several. Collapse each
  // revision family (they all share a parentEstimateId, or are their own root)
  // to a single entry, and show the revision you'd actually work on: the
  // editable one if there is one, otherwise the newest. Picking a different
  // revision happens inside the estimate.
  const visibleEstimates = useMemo(() => {
    const families = new Map<string, Estimate[]>();
    for (const estimate of filteredEstimates) {
      ...
      if (family) family.push(estimate);
      else families.set(rootId, [estimate]);
    }

    const primaries = Array.from(families.values()).map((family) => {
      const ordered = [...family].sort((a, b) => a.version - b.version);
      const contract = ordered.find((e) => e.status === "contract");
      const editable = ordered.find((e) => !e.isLocked);
      return contract ?? editable ?? ordered[ordered.length - 1];
    });

    // The signed contract leads — it's the one that matters at a glance.
    return primaries.sort((a, b) => {
      const rank = (e: Estimate) => (e.status === "contract" ? 0 : 1);
      return rank(a) - rank(b);
    });
  }, [filteredEstimates]);

  /** How many revisions sit behind each visible entry. */
  ...
  const revisionCountFor = (estimate: Estimate) =>
    revisionCounts.get(estimate.parentEstimateId || estimate.id) || 1;
  ...
  }, [estimateStatuses, visibleEstimates]);
  ...
  const estimateColumns = useMemo<ColumnDef<Estimate, unknown>[]>(() => [
    {
      ...
      accessorFn: (e) => e.name || "",
      cell: ({ row }) => {
        const revisions = revisionCountFor(row.original);
        return (
          <div className="flex items-center gap-1.5 min-w-0" data-testid={`cell-name-${row.original.id}`}>
            <span className="text-xs font-medium line-clamp-1">{row.original.name}</span>
            {revisions > 1 && (
              <span
                className="text-data text-muted-foreground flex-shrink-0"
                title={`${revisions} revisions — open the estimate to switch between them`}
              >
                {getRevLabel(row.original.version)}
              </span>
            )}
          </div>
        );
    ...
    // Inside a project the Project column says the same thing on every row.
    ...(scoped ? [] : [{
      id: "project",
      header: "Project",
      accessorFn: (e: Estimate) => getProjectName(e.projectId),
  ...
  const handleRowClick = (estimate: Estimate) => {
    ...
    setLocation(`/projects/${estimate.projectId}/estimates/${estimate.id}?from=${encodeURIComponent(selfPath)}`);
  };


  return (
    <div className="flex flex-col h-full" data-testid="estimates-page">
      {/* Breadcrumb — omitted when embedded; the project shell has its own. */}
      {!embedded && (
      ...
      {/* Single toolbar — search and filtering on the left, view and actions
          on the right. These were two rows; one does the same job. */}
      <div className="h-9 bg-background flex items-center justify-between px-2 gap-2 border-b border-border flex-shrink-0">
        ...
        {/* Right: view toggle + column picker + actions */}
        <div className="flex items-center gap-1.5 flex-shrink-0">
        {/* View toggle (icon only). Hidden in the archive, which has no
            ...
            {effectiveView === 'kanban' && (
              ...
              >
                <div className="flex gap-4 overflow-x-auto pb-4">
                  {boardColumns
                    .map(status => {
                      const columnEstimates = visibleEstimates.filter(est => est.status === status.key);
                      ...
                      return (
                        <KanbanColumn
                          ...
                          projects={projects}
                          returnPath={selfPath}
                          revisionCountFor={revisionCountFor}
                          cardWidth={cardWidth}
...
function KanbanColumn({ status, estimates, projects, cardWidth, isHighlighted, returnPath, revisionCountFor }: {
  status: FieldOption;
  estimates: Estimate[];
  projects: Project[];
  returnPath: string;
  revisionCountFor: (e: Estimate) => number;
  cardWidth: CardWidth;
          ...
          <SortableContext id={status.key} items={estimates.map(e => e.id)} strategy={verticalListSortingStrategy}>
            {estimates.map(estimate => (
              <SortableEstimateCard
                ...
                revisionCount={revisionCountFor(estimate)}
              />
            ))}
          </SortableContext>
        </div>
...
function SortableEstimateCard({ estimate, projects, returnPath, revisionCount }: {
  estimate: Estimate;
  projects: Project[];
  returnPath: string;
  ...
  const handleEstimateClick = (e: React.MouseEvent) => {
    ...
    setLocation(`/projects/${estimate.projectId}/estimates/${estimate.id}?from=${encodeURIComponent(returnPath)}`);
  };
