...
// Gantt Chart Stages (parent-level timeline items)
export const ganttStages = pgTable("gantt_stages", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  projectId: varchar("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
  
  // Core stage information
  name: text("name").notNull(), // "Framing", "Electrical", "Plumbing"
  description: text("description"),
  ...
  color: text("color").default("#bba7db"), // Lilac by default
  displayOrder: integer("display_order").notNull().default(0),
  isCollapsed: boolean("is_collapsed").notNull().default(false), // For UI state
...
export const insertGanttStageSchema = createInsertSchema(ganttStages).omit({
  id: true,
  companyId: true,
  createdAt: true,
  updatedAt: true,
...
export type InsertGanttStage = z.infer<typeof insertGanttStageSchema>;
export type GanttStage = typeof ganttStages.$inferSelect;

// Gantt Chart Subtasks (child items under stages)
export const ganttSubtasks = pgTable("gantt_subtasks", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  stageId: varchar("stage_id").notNull().references(() => ganttStages.id, { onDelete: "cascade" }),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  projectId: varchar("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
  
  // Core subtask information
  name: text("name").notNull(), // "Walls", "Bracing", "Roof Trusses"
  description: text("description"),
...
export const insertGanttSubtaskSchema = createInsertSchema(ganttSubtasks).omit({
  id: true,
  companyId: true,
  createdAt: true,
  updatedAt: true,
...
export type InsertGanttSubtask = z.infer<typeof insertGanttSubtaskSchema>;
export type GanttSubtask = typeof ganttSubtasks.$inferSelect;
