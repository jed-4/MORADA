...
- **Calendar System** (October 2025): Dual calendar structure with personal and business views. Personal calendar (accessible via header icon button) shows user's tasks across all projects with completion checkboxes and visual states. Business calendar (in business sidebar) displays all company events with project and user filtering. Both calendars integrate tasks and schedule items with month/week/day views, drag-and-drop support, and real-time status updates. Calendar removed from project sidebar as schedules are managed per-project.
  - **Saved Views & Filtering** (October 2025): Notion-style flexible filtering system for both calendars. Users can filter by projects, status, event types (tasks/schedule items/Google Calendar), assignees, and date ranges. Saved views allow users to create, edit, and delete custom filter configurations with persistent storage per calendar type. Features: multi-select checkboxes for projects (with scrollable containers), status filter using field options (uses "name" field for display), assignee filtering (excludes invalid users without name/email), date range pickers, and event type toggles. Layout optimization: filters and saved views moved to top header bar (outside calendar card) for cleaner UI, removed event count badges. Default "All Events" view created automatically on first load.
