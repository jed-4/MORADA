...
- **Task Management**: Kanban, List, and Calendar views with drag-and-drop, task templates, inline checklist management, and due date filtering. Polymorphic task context model for consistent handling across projects and business.
  ...
  - **Duplicate Task**: Tasks can be duplicated via 3-dot menu "Duplicate" option. Opens new task modal with copied data (title appended with "(Copy)", same status, priority, assignees, labels, checklist items) but no ID. Uses `onDuplicate` callback from TaskEditModal and `initialData` prop for pre-populating form.
  - **Reminder on Task Creation**: Reminders can be set for new tasks before saving. Uses pending reminder pattern: `SetReminderDialog` calls `onPendingReminder` callback to store reminder config in `pendingReminder` state, then reminder is created after task save with the new task ID. Bell icon shows amber color when pending reminder is set.
  - **Reminder Notifications**: When reminders fire, `reminderProcessor.ts` creates entries in the main `notifications` table so they appear in the NotificationBell. Task links use `?taskId=xxx` query params. Project tasks: `/projects/{projectId}/tasks?taskId=xxx`, business tasks: `/business/tasks?taskId=xxx`. NotificationBell uses full navigation for query-param links to ensure component remount. Tasks/BusinessTasks pages use `useLocation` reactive effects to parse taskId and auto-open task modals, supporting both full reloads and client-side navigation.
- **Site Diary System**: Company-wide templates define form structure (field types: text, textarea, number, date, select, checkbox, file, photo-gallery). Project-specific entries record daily site activities. Features: template import/export (Excel), search within field values, checkbox accountability tracking (who checked, when), file/photo uploads via Object Storage presigned URLs, weather data recording, delete with permission checks (`projects.site_diary`). User story doc: `docs/user-stories/03-site-diaries.md`.
...
- **Timesheets System**: Global access, compact table design, configurable columns, tabbed views (Table/Weekly/Calendar), rapid approval modal. **Date Format Setting**: Company-level toggle between short ("1/1/26") and long ("Mon 1 Jan") date display formats via `useTimesheetDateFormat` hook and `formatTimesheetDate` utility. **Subcontractor Workflow**: Users can be flagged as subcontractors (`isSubcontractor` on users table) with management-only pay rate (`hourlyRate`) and charge rate (`chargeRate`). Subcontractor timesheets: $0 cost (hourlyRate=0, total=$0), hours still tracked. On approval, `poStatus` auto-set to "awaiting_po". PO generation dialog creates purchase orders from approved subcontractor timesheets with line items showing Date/Project/Time/CostCode. PO status column in timesheet table shows Awaiting PO / On PO / Paid. When PO status changes to "billed", linked timesheet entries auto-update to "paid".
