...
- **Calendar System** (October 2025): Dual calendar structure with personal and business views. Personal calendar (accessible via header icon button) shows user's tasks across all projects with completion checkboxes and visual states. Business calendar (in business sidebar) displays all company events with project and user filtering. Both calendars integrate tasks and schedule items with month/week/day views, drag-and-drop support, and real-time status updates. Calendar removed from project sidebar as schedules are managed per-project.
  - **Saved Views & Filtering** (October 2025): Notion-style flexible filtering system for both calendars. Saved views allow users to create, edit, and delete custom filter configurations with persistent storage per calendar type. Users can configure filters for projects, status, event types (tasks/schedule items/Google Calendar), assignees, and date ranges within the saved view creation/edit dialog or via Filters button. Features: multi-select checkboxes for projects (with scrollable containers), status filter using field options (uses "name" field for display), assignee filtering (excludes invalid users without name/email), date range pickers, and event type toggles. Layout optimization: saved views and filters buttons in top header bar (outside calendar card) for cleaner UI, removed event count badges and active filter badges to prevent visual chaos from elements popping in/out. Default "All Events" view created automatically on first load.
  - **Schema Fix** (October 2025): Fixed calendar view creation validation where `insertCalendarViewSchema` incorrectly required `userId` and `companyId` in request body. These server-injected fields are now omitted from validation schema, allowing backend to inject them from session after validation. Pattern matches other server-populated field schemas throughout codebase.
