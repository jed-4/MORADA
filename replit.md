...
- **Proposals System**: Complete proposal management workflow from creation to client acceptance and invoice conversion. Database schema with 4 tables (proposals, proposal_sections, proposal_items, proposal_acceptances). Backend features comprehensive storage layer with CRUD operations, workflow API endpoints for status transitions (draft → sent → viewed → accepted/rejected), acceptance tracking with audit trail, and proposal-to-invoice conversion with validation guards. Frontend includes proposals list page with grid/kanban views, search/filtering, status badges with drag-drop updates, and proposal creation/edit page with estimate import functionality, sections/items builder, and form validation. Client acceptance flow tracks viewedDate, acceptedBy, acceptedByName, acceptedByEmail, and digital signature. Supports introductionText, closingText, terms & conditions, expiry dates, and GST calculations.
